...
**Boot time (27ms)** — fourth-fastest out of six. Faster than Ruby and well ahead of the BEAM. Acceptable for a scripting/application runtime.

**I/O concurrency (http, 1.2× Node)** — Brood's green-process model handles concurrent I/O well. 500 in-flight HTTP requests completes in 152ms, essentially matching .NET (132ms) and close to Node (123ms). The actor-style network stack is working.

**Sort (36ms, 2× Node)** — Brood's sort stays within 2–3× of all other runtimes. Suggests the built-in sort algorithm and list representation are reasonably efficient.
...
### 1. Tight loop / per-instruction overhead — collatz 75×, loop 88×, matmul 665×

The three worst compute results all share the same root cause: the VM pays a fixed overhead per operation that compounds over many iterations. Collatz (299ms vs .NET's 4ms), plain loop (176ms vs .NET's 2ms), and matmul (665ms vs .NET's 1ms) are dominated by raw bytecode dispatch cost, not algorithmic complexity.

Integer arithmetic was improved in this run (ADR: `Prim2SlotSlot`/`Prim2SlotInt` fusion + `prim2_int_fast` inline fast path), bringing loop from 199ms to 176ms and collatz from 327ms to 299ms.
...
- `defn` self-recursion via `SelfCall`. Currently each recursive iteration re-resolves the callee through the global IC; emitting `Inst::SelfCall` for `defn` tail positions would eliminate this.
...
### 4. Function call overhead — fib 75×, reduce 102×

Recursive fib(30) is 75× behind .NET. The call stack for fib(30) is ~1.5 M recursive calls; the per-call overhead is roughly 150ns vs .NET's ~2ns. This is inline with an unoptimised interpreter call frame.
...
### 5. GC / allocation throughput — bintree 55×

Building and walking `2^40` binary tree nodes stresses allocation rate and GC pause. Brood's 386ms vs Node's 7ms suggests either allocation is slow or GC pauses are large.
...
37× behind .NET and 29× behind Python on a join + length of 50k strings. Immutable string concatenation in a loop is quadratic unless the runtime detects it; if Brood's `join` is implemented as repeated append this is the dominant cost.
...
|------|--------------------------|--------------------|--------------|------------------------|
| tight loops | collatz, loop, matmul | 75–665× | per-instruction VM overhead | very high |
| array indexing | matmul | 665× | no O(1) array type | high |
| parallel scheduling | pfib, spawn | 47–48× | processes not distributed across cores | high |
| function call | fib, reduce | 75–102× | call frame cost | medium |
| GC / allocation | bintree | 55× | allocation rate or GC pause | medium |
