...
**I/O concurrency (http, 1.3× Node)** — Brood's green-process model handles concurrent I/O well. 500 in-flight HTTP requests completes in 176ms (wall), close to .NET (123ms) and Node (130ms). The actor-style network stack is working.

**Sort (61ms wall, ~2× Node)** — Brood's sort stays within ~2× of most runtimes. Suggests the built-in sort algorithm and list representation are reasonably efficient.
...
### 1. Tight loop / per-instruction overhead — collatz 66×, loop 111×, matmul 372×
...
**A note on variance:** an earlier 2026-06-09 run showed `loop` spiking to 229ms compute; a re-run brought it back to ~155ms, so that spike was a measurement transient (system load), not a real regression. The single-threaded compute benchmarks here are stable across runs to within a few percent. The genuinely high-variance results are the concurrency benchmarks (`spawn`, `pfib`, `http`), which swing 15–45% between runs with scheduler / CPU contention — read those as ballpark, not precise.
...
- This would also improve bintree (100×) and any future numeric workload.

### 3. Parallel scheduling — pfib 57×, spawn 67×

`pfib` (100 × fib(28) in parallel) takes 2.25s for Brood vs 39ms for .NET. The work is fully independent; on 12 cores this should take ~fib(28)/12 ≈ 180ms. The result suggests Brood's scheduler is not distributing green processes across cores.

`spawn` (20k processes each computing fib(15)) is 1.36s vs Elixir's 97ms.
...
### 4. Function call overhead — fib 68×, reduce 95×

Recursive fib(30) is 68× behind .NET. The call stack for fib(30) is ~1.5 M recursive calls; the per-call overhead is roughly 150ns vs .NET's ~3ns.

`reduce` (fold over 1 M elements with a lambda) at 95× shows that higher-order calls compound this further.
...
### 5. GC / allocation throughput — bintree 100×

Building and walking `2^40` binary tree nodes stresses allocation rate and GC pause. Brood's 402ms vs Node's 6ms suggests either allocation is slow or GC pauses are large.
...
### 6. String operations — strings 35×

35× behind .NET on a join + length of 50k strings.
...
|------|--------------------------|--------------------|--------------|------------------------|
| tight loops | collatz, loop, matmul | 66–372× | per-instruction VM overhead | very high |
| array indexing | matmul | 372× | no O(1) array type | high |
| parallel scheduling | pfib, spawn | 57–67× | processes not distributed across cores | high |
| function call | fib, reduce | 68–95× | call frame cost | medium |
| GC / allocation | bintree | 100× | allocation rate or GC pause | medium |
| string ops | strings | 35× | non-linear join | medium |
