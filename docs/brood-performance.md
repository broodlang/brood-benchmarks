...
Based on the whklat benchmark run (2026-06-10). Numbers are compute time (wall − boot).
...
**I/O concurrency (http, 1.5× Node)** — Brood's green-process model handles concurrent I/O well. 500 in-flight HTTP requests complete in 208ms (wall), 3rd of six behind Node (140ms) and .NET (175ms), well ahead of Python/Ruby/Elixir. The actor-style network stack is working. (This row was invalid in earlier runs — every client was reaching a stray process on a fixed port rather than the benchmark server; the harness now binds a free port and verifies the server is its own.)
...
### 1. Tight loop / per-instruction overhead — collatz 53×, loop 70×, matmul 301×
...
**A note on variance:** an earlier 2026-06-09 run showed `loop` spiking to 229ms compute; clean re-runs sit at 130–155ms, so that spike was a measurement transient (system load), not a real regression. The single-threaded compute benchmarks are stable across runs to within a few percent. The high-variance results are the concurrency benchmarks (`spawn`, `pfib`, `http`), which swing with scheduler / CPU contention — the harness now takes the best of 7 runs for these (vs 3 for the rest) to tighten the floor, but read them as ballpark.
...
- This would also improve bintree (85×) and any future numeric workload.

### 3. Parallel scheduling — pfib 55×, spawn 63×

`pfib` (100 × fib(28) in parallel) takes 2.20s for Brood vs 40ms for .NET. The work is fully independent; on 12 cores this should take ~fib(28)/12 ≈ 180ms. The result suggests Brood's scheduler is not distributing green processes across cores.

`spawn` (20k processes each computing fib(15)) is 1.32s vs Elixir's 78ms.
...
### 4. Function call overhead — fib 54×, reduce 61×
...
`reduce` (fold over 1 M elements with a lambda) at 61× shows that higher-order calls compound this further.
...
### 5. GC / allocation throughput — bintree 85×

Building and walking `2^40` binary tree nodes stresses allocation rate and GC pause. Brood's 401ms vs Node's 7ms suggests either allocation is slow or GC pauses are large.
...
### 6. String operations — strings 32×

32× behind .NET on a join + length of 50k strings.
...
|------|--------------------------|--------------------|--------------|------------------------|
| tight loops | collatz, loop, matmul | 53–301× | per-instruction VM overhead | very high |
| array indexing | matmul | 301× | no O(1) array type | high |
| parallel scheduling | pfib, spawn | 55–63× | processes not distributed across cores | high |
| function call | fib, reduce | 54–61× | call frame cost | medium |
| GC / allocation | bintree | 85× | allocation rate or GC pause | medium |
| string ops | strings | 32× | non-linear join | medium |
