...
**Boot time (29 ms)** — fourth-fastest of six, ahead of Ruby and far ahead of the BEAM. Fine for a scripting/application runtime.

**I/O concurrency (http, 1.4× Node)** — the green-process model handles concurrent I/O well: 500 in-flight GETs complete in 203 ms (wall), 3rd of six behind Node (142 ms) and .NET (175 ms), well ahead of Python/Ruby/Elixir. The actor-style network stack is working.
...
### 1. Tight loop / per-instruction overhead — loop 147×, collatz 85×, matmul 152×
...
### 2. Array indexing — addressed (matmul 301× → 152×)
...
### 3. Parallel scheduling — pfib 63×, spawn 49×
...
### 4. Function call overhead — fib 74×
...
### 5. GC / allocation throughput — bintree 129×

Building and walking many short-lived tree nodes stresses allocation rate and GC pause (336 ms vs Node's 7 ms). A bump-pointer nursery for small, short-lived objects would cut per-node allocation cost; the inline `vector-ref` already shaved ~1.2× off the walk.

### 6. String operations — improved (strings 32× → 16×)
...
| area | representative benchmark | vs fastest | status / likely cause |
|------|--------------------------|-----------|------------------------|
| tight loops | loop, collatz, matmul | 85–152× | open — per-op VM overhead; JIT subset gaps (Global, back-edge tiering) |
| array indexing | matmul | 152× | **addressed** — `nth`/`vector-ref` inlined; residual is per-op overhead |
| parallel scheduling | pfib, spawn | 49–63× | open — processes likely not spread across cores |
| function call | fib | 74× | open — non-tail dispatch cost; JIT tier-2 call lowering may help |
| GC / allocation | bintree | 129× | open — per-node allocation / GC pauses |
| string ops | strings | 16× | **addressed** — native `%string-join`; residual is `number->string` |
| higher-order fold | reduce | 21× | **addressed** — primitive-reducer fast path |
