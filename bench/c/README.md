...
| `errors`, `errors-deep` | C has no exceptions, and the row measures **unwinding** — 50 non-tail frames thrown through and recovered at the top. `setjmp`/`longjmp` restores a saved context and jumps, discarding the intervening frames without visiting them, so it would win the row by not performing the operation the row exists to measure. An error-code return checked at all 50 levels is what C really does, but that is branch-and-return cost, a different row. **Settled exclusion (2026-08-14).** |
