...
(import '[java.net.http HttpClient HttpRequest HttpResponse$BodyHandlers]
        '[java.net URI])
(def n (long (Long/parseLong (or (System/getenv "BENCH_N") "100"))))
(def port (or (System/getenv "BENCH_HTTP_PORT") "8089"))
(def url (str "http://127.0.0.1:" port "/"))
(def ^HttpClient client (HttpClient/newHttpClient))
(defn fetch [_]
  (try
    (let [req (.build (.uri (HttpRequest/newBuilder) (URI/create url)))
          ^java.net.http.HttpResponse resp (.send client req (HttpResponse$BodyHandlers/ofString))]
