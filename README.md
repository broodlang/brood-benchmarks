...
`fib` **−89%**, `bintree` **−82%**, `spawn-live` **−73%** (5362 → 1438 ms), `ring` **−62%**,
...
**2026-09-14 run.** Numbers regenerate with each run; the full table is in [BENCHMARKS.md](BENCHMARKS.md).

| | Brood | field |
|---|---|---|
| startup (wall) | 13.4 ms | C 2.0, Python 10.1, Node 17.9, .NET 21.8, Ruby 38.6, Elixir 179.6, Clojure 335.3 ms |
| base RSS | 52.7 MB | C 1.7, Python 9.7, Ruby 19.3, .NET 25.9, Node 42.6, Elixir 73.3, Clojure 104.1 MB |
| overall speed — geomean of the 15 rows every column implements | 10.7× the fastest (4/8) | C 1.0 · .NET 3.0 · Node 5.3 · Elixir 10.7 · Clojure 29.3 · Ruby 35.7 · Python 51.1 |
| aggregate single-threaded compute | 5.7× the fastest | C 1.0 · .NET 2.0 · Node 4.4 · Elixir 7.3 · Clojure 14.6 · Ruby 24.6 · Python 52.9 |
| aggregate vs the field's average | **0.77×** | ahead of the field average on 6 of 11 core-compute rows |
| rank by row | 1st on —; last on `spawn-live` | |
| `spawn-live` (300k units held alive, each sent a copied message) | 1.45 s, 1.68 GB, 4.33 CPU·s | Elixir 723 ms / 911 MB — the only peer; Node 217 ms / 243 MB, .NET 284 ms / 141 MB, Python 1.26 s / 361 MB are coroutines on a shared heap |
| `supervisor` (20k supervised children, a quarter retired and restarted) | 848 ms, 689 MB | Elixir 265 ms / 156 MB — the only peer; no other runtime here has a supervisor |
| `latency` p99 / p99.9 (20k req/s, 5% of them occupying 500µs) | **81 µs / 367 µs** | Elixir 51 / 87 µs · Python 449 / 472 µs · Node 450 / 521 µs · .NET 740 / 12520 µs — the compute winner has the worst tail |
<!-- END STANDINGS -->
...
Numbers in the docs were measured on `whklat`: a 12-core x86-64 Linux 7.0.0 machine · Brood 0.27.2 (`c813ce1a`) (bytecode VM + tier-1 JIT) · Clojure 1.12.5 / OpenJDK 25.0.4 (HotSpot) · Elixir 1.21.0-dev (b82c44a) / OTP 28 (BeamAsm JIT) · Python 3.14.4 · Node 22.21.0 (V8) · ruby 3.3.8 · .NET 10.0.112 (RyuJIT). 2026-09-14.
<!-- END ENVIRONMENT -->
