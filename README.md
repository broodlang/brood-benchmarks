...
| base RSS | 66.1 MB | C 1.5, Python 9.6, Ruby 19.0, .NET 25.9, Node 42.7, Elixir 71.7, Clojure 103.9 MB |
...
| aggregate vs the field's average | **0.79×** | ahead of the field average on 6 of 11 core-compute rows |
| rank by row | 1st on —; last on `spawn-live` | |
| `spawn-live` (300k units held alive, each sent a copied message) | 1.58 s, 1.67 GB, 4.61 CPU·s | Elixir 709 ms / 922 MB — the only peer; Node 219 ms / 243 MB, .NET 276 ms / 139 MB, Python 1.27 s / 361 MB are coroutines on a shared heap |
| `supervisor` (20k supervised children, a quarter retired and restarted) | 884 ms, 636 MB | Elixir 266 ms / 157 MB — the only peer; no other runtime here has a supervisor |
| `latency` p99 / p99.9 (20k req/s, 5% of them occupying 500µs) | **85 µs / 683 µs** | Elixir 64 / 82 µs · Node 454 / 517 µs · Python 459 / 604 µs · .NET 779 / 13185 µs — the compute winner has the worst tail |
<!-- END STANDINGS -->
...
Numbers in the docs were measured on `whklat`: a 12-core x86-64 Linux 7.0.0 machine · Brood 0.27.0 (`7b0327fb`) (bytecode VM + tier-1 JIT) · Clojure 1.12.5 / OpenJDK 25.0.4 (HotSpot) · Elixir 1.21.0-dev (b82c44a) / OTP 28 (BeamAsm JIT) · Python 3.14.4 · Node 22.21.0 (V8) · ruby 3.3.8 · .NET 10.0.111 (RyuJIT). 2026-08-28.
<!-- END ENVIRONMENT -->
