...
| | Brood | field |
|---|---|---|
| startup (wall) | 20.0 ms | C 1.9, Python 10.1, Node 18.0, .NET 21.5, Ruby 38.6, Elixir 183.4, Clojure 340.3 ms |
| base RSS | 59.7 MB | C 1.5, Python 9.6, Ruby 19.0, .NET 25.9, Node 42.7, Elixir 71.7, Clojure 103.9 MB |
...
| `spawn-live` (300k units held alive, each sent a copied message) | 1.45 s, 1.66 GB, 4.36 CPU·s | Elixir 709 ms / 922 MB — the only peer; Node 219 ms / 243 MB, .NET 276 ms / 139 MB, Python 1.27 s / 361 MB are coroutines on a shared heap |
| `supervisor` (20k supervised children, a quarter retired and restarted) | 836 ms, 620 MB | Elixir 266 ms / 157 MB — the only peer; no other runtime here has a supervisor |
| `latency` p99 / p99.9 (20k req/s, 5% of them occupying 500µs) | **86 µs / 716 µs** | Elixir 64 / 82 µs · Node 454 / 517 µs · Python 459 / 604 µs · .NET 779 / 13185 µs — the compute winner has the worst tail |
<!-- END STANDINGS -->
...
Numbers in the docs were measured on `whklat`: a 12-core x86-64 Linux 7.0.0 machine · Brood 0.18.1 (`c890fd59`) (bytecode VM + tier-1 JIT) · Clojure 1.12.5 / OpenJDK 25.0.4 (HotSpot) · Elixir 1.21.0-dev (b82c44a) / OTP 28 (BeamAsm JIT) · Python 3.14.4 · Node 22.21.0 (V8) · ruby 3.3.8 · .NET 10.0.111 (RyuJIT). 2026-08-28.
<!-- END ENVIRONMENT -->
