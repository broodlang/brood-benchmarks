...
**2026-08-27 run.** Numbers regenerate with each run; the full table is in [BENCHMARKS.md](BENCHMARKS.md).

| | Brood | field |
|---|---|---|
| startup (wall) | 18.3 ms | C 1.9, Python 10.0, Node 17.8, .NET 21.9, Ruby 39.2, Elixir 183.5, Clojure 339.9 ms |
| base RSS | 52.0 MB | C 1.6, Python 9.8, Ruby 19.0, .NET 25.8, Node 42.7, Elixir 72.5, Clojure 103.7 MB |
| overall speed — geomean of the 15 rows every column implements | 10.6× the fastest (4/8) | C 1.0 · .NET 3.1 · Node 5.2 · Elixir 10.8 · Clojure 31.2 · Ruby 36.4 · Python 52.1 |
| aggregate single-threaded compute | 6.1× the fastest | C 1.0 · .NET 2.0 · Node 4.4 · Elixir 7.5 · Clojure 15.2 · Ruby 24.7 · Python 56.2 |
| aggregate vs the field's average | **0.80×** | ahead of the field average on 6 of 11 core-compute rows |
| rank by row | 1st on —; last on `spawn-live` | |
| `spawn-live` (300k units held alive, each sent a copied message) | 1.37 s, 1.71 GB, 4.22 CPU·s | Elixir 695 ms / 913 MB — the only peer; Node 225 ms / 244 MB, .NET 294 ms / 138 MB, Python 1.30 s / 361 MB are coroutines on a shared heap |
| `supervisor` (20k supervised children, a quarter retired and restarted) | 834 ms, 649 MB | Elixir 258 ms / 156 MB — the only peer; no other runtime here has a supervisor |
| `latency` p99 / p99.9 (20k req/s, 5% of them occupying 500µs) | **87 µs / 397 µs** | Elixir 62 / 89 µs · .NET 450 / 12865 µs · Node 462 / 561 µs · Python 469 / 609 µs — the compute winner has the worst tail |
<!-- END STANDINGS -->
...
Numbers in the docs were measured on `whklat`: a 12-core x86-64 Linux 7.0.0 machine · Brood 0.13.0 (`8d7f5fa7`) (bytecode VM + tier-1 JIT) · Clojure 1.12.5 / OpenJDK 25.0.4 (HotSpot) · Elixir 1.21.0-dev (b82c44a) / OTP 28 (BeamAsm JIT) · Python 3.14.4 · Node 22.21.0 (V8) · ruby 3.3.8 · .NET 10.0.111 (RyuJIT). 2026-08-27.
<!-- END ENVIRONMENT -->
