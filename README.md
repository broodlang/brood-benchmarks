...
`pingpong` **−43%** — and one line going the other way, `supervisor` **+58%**, which is the open
regression described in [BENCHMARKS.md](BENCHMARKS.md#concurrency--only-two-of-these-are-the-same-thing).
...
| base RSS | 51.0 MB | C 1.7, Python 9.7, Ruby 19.3, .NET 25.9, Node 42.6, Elixir 73.3, Clojure 104.1 MB |
...
| `spawn-live` (300k units held alive, each sent a copied message) | 1.43 s, 1.69 GB, 4.29 CPU·s | Elixir 723 ms / 911 MB — the only peer; Node 217 ms / 243 MB, .NET 284 ms / 141 MB, Python 1.26 s / 361 MB are coroutines on a shared heap |
| `supervisor` (20k supervised children, a quarter retired and restarted) | 1.33 s, 644 MB | Elixir 265 ms / 156 MB — the only peer; no other runtime here has a supervisor |
| `latency` p99 / p99.9 (20k req/s, 5% of them occupying 500µs) | **82 µs / 280 µs** | Elixir 51 / 87 µs · Python 449 / 472 µs · Node 450 / 521 µs · .NET 740 / 12520 µs — the compute winner has the worst tail |
<!-- END STANDINGS -->
...
Numbers in the docs were measured on `whklat`: a 12-core x86-64 Linux 7.0.0 machine · Brood 0.27.2 (`c9d6c1a1`) (bytecode VM + tier-1 JIT) · Clojure 1.12.5 / OpenJDK 25.0.4 (HotSpot) · Elixir 1.21.0-dev (b82c44a) / OTP 28 (BeamAsm JIT) · Python 3.14.4 · Node 22.21.0 (V8) · ruby 3.3.8 · .NET 10.0.112 (RyuJIT). 2026-09-14.
<!-- END ENVIRONMENT -->
