...
against Elixir's ~3.1 KB). **This run it did not move** — the first flat result after four
consecutive improvements, and flat by measurement rather than by absence of evidence: a
fixed-baseline A/B against 0.3.9 reads CPU **+1.1%** and wall **−0.6%** against the row's own
0.9% noise floor. The four earlier moves came from keying compiled-code sharing by AST, from
`fold` walking a vector by index, and finally from folding the payload in a native counted loop
that resolves a passthrough reducer like `+` once instead of per element.

What remains is the **process floor** — the largest single piece, and unchanged by any of those
four — plus the cost of *reaching* the fold at all: a cold call into the Brood-level `fold`
wrapper is now larger than the fold it performs. The **receive machinery** was the standing
suspect here and has been measured and dropped: a matcher whose pattern compares a literal really
does miss the JIT's native fast frame, but making it native is worth ~2% of this row, inside the
run-to-run noise. See [FRONTIER.md](FRONTIER.md).
