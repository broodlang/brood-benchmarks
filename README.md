...
> dispatched calls. Two recent runtime fixes show in the numbers: a
> **process-count-aware GC floor** cut parallel fan-out's peak memory ~35×
> (`pfib` ~980 MB → ~27 MB), and **routing spawned processes through the VM**
> (they were tree-walked even under the VM) cut `pfib`'s wall time ~4×
> (3.84 s → ~1 s).
...
**The short version:** Brood's strengths are **memory** (~11 MB base, far
lighter than the rest on most workloads) and **fast-enough startup** (~25 ms,
~13× ahead of the BEAM); it beats Elixir end-to-end on the short tasks and **runs
in the front group on concurrent I/O** (a hair behind Node, ahead of Python and
Ruby). Its main weakness is **raw single-threaded compute** — the young bytecode
VM trails not only Node's JIT (~16–34×) but Ruby (~4–14×) and Python (~3–7× on
tight loops, more on allocation-heavy work), both also interpreted. Parallel CPU
(`pfib`) was once a disaster (30× off, ~980 MB) and is now merely last (~2–7× off
the field) while staying among the lightest, after two runtime fixes. See
...
· Ubuntu 26.04 · Brood 0.1.0 (bytecode VM, process-count-aware GC floor,
VM-routed spawned processes) · Elixir 1.20.0 / OTP 29 · Python 3.14.4 · Node
24.15.0 · Ruby 3.3.8. Best of 3 runs each (latency-sensitive `startup`/`http`
isolated, best of 5).
