...
Per-row, each against its own first published run — immune to the rebase above, which is why
this is the chart to read for progress rather than the scalar. `spawn-live` is **−67%** across
every published run at this size (5362 → 1795 ms). Rows are only compared across runs at the
same `N`.

<!-- BEGIN STANDINGS (generated by bench/docs.py) -->
**2026-08-25 run.** Numbers regenerate with each run; the full table is in [BENCHMARKS.md](BENCHMARKS.md).

| | Brood | field |
|---|---|---|
| startup (wall) | 29.9 ms | C 1.9, Python 10.0, Node 17.6, .NET 21.7, Ruby 38.9, Elixir 181.2, Clojure 334.1 ms |
| base RSS | 56.8 MB | C 1.7, Python 9.7, Ruby 19.1, .NET 25.9, Node 42.3, Elixir 70.3, Clojure 103.4 MB |
| overall speed — geomean of the 15 rows every column implements | 10.1× the fastest (4/8) | C 1.0 · .NET 3.1 · Node 5.6 · Elixir 10.3 · Clojure 33.2 · Ruby 36.2 · Python 53.2 |
| aggregate single-threaded compute | 6.0× the fastest | C 1.0 · .NET 2.1 · Node 5.5 · Elixir 7.3 · Clojure 16.8 · Ruby 24.8 · Python 60.0 |
| aggregate vs the field's average | **0.74×** | ahead of the field average on 7 of 11 core-compute rows |
| rank by row | 1st on —; last on `spawn-live` | |
| `spawn-live` (300k units held alive, each sent a copied message) | 1.42 s, 1.70 GB, 4.33 CPU·s | Elixir 692 ms / 918 MB — the only peer; Node 216 ms / 244 MB, .NET 272 ms / 138 MB, Python 1.41 s / 361 MB are coroutines on a shared heap |
| `supervisor` (20k supervised children, a quarter retired and restarted) | 806 ms, 627 MB | Elixir 262 ms / 151 MB — the only peer; no other runtime here has a supervisor |
| `latency` p99 / p99.9 (20k req/s, 5% of them occupying 500µs) | **86 µs / 403 µs** | Elixir 58 / 90 µs · Node 455 / 549 µs · Python 488 / 644 µs · .NET 698 / 12631 µs — the compute winner has the worst tail |
<!-- END STANDINGS -->
...
Numbers in the docs were measured on `whklat`: a 12-core x86-64 Linux 7.0.0 machine · Brood 0.12.0 (`c39ca87c`) (bytecode VM + tier-1 JIT) · Clojure 1.12.5 / OpenJDK 25.0.3 (HotSpot) · Elixir 1.21.0-dev (b82c44a) / OTP 28 (BeamAsm JIT) · Python 3.14.4 · Node 22.21.0 (V8) · ruby 3.3.8 · .NET 10.0.111 (RyuJIT). 2026-08-25.
<!-- END ENVIRONMENT -->
