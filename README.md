...
**2026-08-06 run.** Numbers regenerate with each run; the full table is in [BENCHMARKS.md](BENCHMARKS.md).

| | Brood | field |
|---|---|---|
| startup (wall) | 17.6 ms | Python 9.9, Node 18.0, .NET 22.3, Ruby 39.3, Elixir 186.9, Clojure 340.3 ms |
| base RSS | 22.1 MB | Python 9.8, Ruby 19.3, .NET 26.1, Node 44.7, Elixir 71.4, Clojure 102.5 MB |
| aggregate single-threaded compute | 2.9× the fastest | .NET 1.0 · Node 2.8 · Elixir 3.5 · Clojure 8.2 · Ruby 12.0 · Python 29.4 |
...
| `spawn-live` (300k units held alive, each sent a copied message) | 1.93 s, 1.57 GB, 5.55 CPU·s | Elixir 699 ms / 917 MB — the only peer; Node 228 ms / 250 MB, .NET 282 ms / 138 MB, Python 1.28 s / 362 MB are coroutines on a shared heap |
| `supervisor` (20k supervised children, a quarter retired and restarted) | 855 ms, 444 MB | Elixir 271 ms / 152 MB — the only peer; no other runtime here has a supervisor |
| `latency` p99 / p99.9 (20k req/s, 5% of them occupying 500µs) | **75 µs / 488 µs** | Elixir 56 / 83 µs · Node 416 / 471 µs · Python 506 / 641 µs · .NET 871 / 13191 µs — the compute winner has the worst tail |
<!-- END STANDINGS -->
...
*copy*. Against its only peer Brood is 2.8× slower and 1.75× heavier (~5.4 KB per live process
against Elixir's ~2.9 KB). It improved again this run, from `fold` walking a vector by index rather
than materialising the tail as a list; the run before, from a shared compiled-code cache that had
been keyed by the closure instance while a no-capture closure is created afresh per spawn. What
remains is the process itself plus the **receive machinery** — a matcher whose pattern compares a
literal never reaches the JIT's native fast frame. See [FRONTIER.md](FRONTIER.md).
...
Numbers in the docs were measured on `whklat`: a 12-core x86-64 Linux 7.0.0 machine · Brood 0.3.0 (`61f3766f`) (bytecode VM + tier-1 JIT) · Clojure 1.12.5 / OpenJDK 25.0.3 (HotSpot) · Elixir 1.21.0-dev (b82c44a) / OTP 28 (BeamAsm JIT) · Python 3.14.4 · Node 22.21.0 (V8) · ruby 3.3.8 · .NET 10.0.110 (RyuJIT). 2026-08-06.
<!-- END ENVIRONMENT -->
