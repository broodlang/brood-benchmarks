...
> (`pfib` ~980 MB → ~100 MB), and **routing spawned processes through the VM**
> (they were tree-walked even under the VM) cut `pfib`'s wall time ~4.7×
> (3.84 s → 0.82 s).
...
lighter than the rest on most workloads) and **fast-enough startup** (~24 ms,
~13× ahead of the BEAM); it beats Elixir end-to-end on the short tasks and **runs
in the front group on concurrent I/O** (just behind Node and Python, ahead of
Ruby). Its main weakness is **raw single-threaded compute** — the young bytecode
VM trails not only Node's JIT (~12–35×) but Ruby (~4–12×) and Python (~2–5×),
both also interpreted. Parallel CPU (`pfib`) was a disaster (30× off, ~980 MB)
and is now merely last (~2–6×) and the lightest in the field, after two runtime
fixes. See [BENCHMARKS.md](BENCHMARKS.md) for the honest, full picture.
